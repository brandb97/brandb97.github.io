import{j as e,c as l,r as d,H as _,F as w}from"./Footer-BCDbRi8S.js";import{C as t,B as h}from"./CodeBox-DHD9yWMe.js";const f="/assets/ibm1969-DPtM4P6B.jpg";function m(){return e.jsxs("div",{children:[e.jsxs("p",{className:"indent",children:["终端是什么？如果你熟悉UNIX系统，你也许会说终端就是",e.jsx("code",{children:'"/dev/tty"'}),"这个设备。的确，从UNIX程序的视角来看，",e.jsx("code",{children:'"/dev/tty"'}),"和普通的文件没有什么区别（试试cat /dev/tty）。在UNIX系统里，一个程序用",e.jsx("code",{children:"open"}),"系统调用既可以打开一个文件，也可以打开一个终端设备。",e.jsx("code",{children:"open"}),"系统调用返回一个整数类型的文件描述符（其实就是一个数组下标），",e.jsx("code",{children:"read/write"}),"系统调用对这个打开的文件描述符进行读写操作。许多时候，对于一个程序来说，一个文件描述符究竟对应于一个磁盘文件还是属于一个终端设备并不重要，只要可以对这个文件描述符进行读写操作就可以。但是操作系统就需要区分文件描述符对应的设备是什么了。如果是读写硬盘，操作系统需要移动硬盘的磁头读写磁道上的数据；如果是读写终端，操作系统的终端驱动程序则需要等待键盘的输入，或者是把ascii码打印在屏幕上。"]}),e.jsxs("p",{className:"indent",children:["所以终端这个名词其实可以对应两种东西：终端设备和终端驱动程序。在很早以前，终端设备，就是指键盘和屏幕，UNIX系统中用",e.jsx("code",{children:'"/dev/tty"'}),"表示；终端驱动则负责实现在终端设备上的系统调用（e.g. UNIX系统上的open，read，write）。下面是一张古早（1969年，来源于维基百科）人们在终端上工作的相片（也许现在有一些性格变态的书呆子也会这么工作，林子大了什么鸟都有！）"]}),e.jsx("div",{className:"ibm1969-pic",children:e.jsx("img",{src:f,alt:"ibm terminal in 1969"})}),e.jsxs("div",{className:"tips",children:["🤔 为什么终端被称为tty？",e.jsx("br",{}),"💡 tty的全称是TeleTYpewriter，如果你想了解更多，请看",e.jsx("a",{href:"https://zh.wikipedia.org/zh-cn/電傳打字機",children:"电传打印机"})]})]})}function x(){return e.jsxs("div",{children:[e.jsx("p",{className:"indent",children:"虽然大部分程序不关心文件描述符是什么设备，但也存在一些程序就是针对终端编写的，比如vim。想想vim要如何实现页面滚动的功能呢？首先，vim需要告诉终端驱动程序关闭行缓冲功能。在默认情况下，终端驱动程序会在用户按下ENTER时才把输入发送给应用程序，但对于vim，如果我们每次要到按“下箭头+ENTER”才能向下滚动显然很不方便。其次，在读取到用户输入的下箭头之后，vim需要结合光标的位置和终端显示器的宽度判断是否需要滚动页面，所以vim还需要通过系统调用获取终端显示器的大小。在UNIX系统中，用来获取和设置终端属性的系统调用如下所示："}),e.jsx(t,{code:`/* 获取终端属性，e.g.是否行缓冲 */
...
stty intr ^L # 设置中断信号对于的字符是CTRL-L`,language:"bash"}),e.jsxs("p",{className:"indent",children:["说了这么多，大家应该明白：除了读写操作，终端文件描述符和普通文件描述符的区别很大。比如我们可以用上面提到的系统调用从终端文件描述符中获取许多普通文件描述符中没有的信息。但是普通文件符能做的",e.jsx("code",{children:"lseek"}),"（将磁盘读写头移到文件的某个指定位置），则对于终端设备没有意义。"]})]})}const p="/assets/window_prog-Disl9x2E.jpeg",j="/assets/window_shell-DmLOIUQH.jpeg",u="/assets/screen_like-uICQQPGK.jpeg";function g(){return e.jsxs("div",{children:[e.jsx("p",{className:"indent",children:"古早时期，窗口=显示器。但现在，我们会在显示器中打开一大堆窗口，每个窗口中运行一个应用程序。一个应用程序的结构可以用下面这幅图片描述"}),e.jsx("div",{style:{textAlign:"center"},children:e.jsx("img",{src:p,alt:"windows program frame",style:{width:"70%"}})}),e.jsx("p",{className:"indent",children:"为了让shell程序也可以运行到窗口上，人们开发了许多专门运行shell命令的窗口程序（比如iTerm，warp等等）。这些程序的结构如下所示"}),e.jsx("div",{style:{textAlign:"center"},children:e.jsx("img",{src:j,alt:"windows shell frame",style:{width:"77%"}})}),e.jsx("p",{className:"indent",children:"看起来非常简单，对不对？不过你也许会问：什么是伪终端（英文简称是pty）？伪终端类似一个UNIX管道，并不是一个真实的设备。在上图中，前端（e.g. MacOS上的iTerm，下文用iTerm代表前端）把用户输入发送到伪终端，命令行程序就可以从stdin读取；命令行程序将输出写入stdout和stderr，iTerm就可以从伪终端中读取输出显示在窗口中。不过，除了具有管道的功能，伪终端还可以像正常的终端一样获取/设置窗口大小，设置各种各样的属性以及处理特殊字符。比如当iTerm被打开时，iTerm就会打开一个伪终端，设置伪终端的窗口大小为iTerm的默认窗口大小。如果iTerm发送CTRL-C到伪终端设备，伪终端设备也不会像管道一样发送CTRL-C供命令行程序读取，而是会发送SIGINT信号给命令行程序。"}),e.jsx("p",{className:"indent",children:"Whew！说了这么多，终于可以谈谈screen这样的窗口管理程序该怎么实现了，一个简单的想法是这样"}),e.jsx("div",{style:{textAlign:"center"},children:e.jsx("img",{src:u,alt:"screen frame",style:{width:"77%"}})}),e.jsxs("p",{className:"indent",children:["这张图片中，screen程序作为前端，读取用户输入只需要在标准输入",e.jsx("code",{children:"STDIN_FILENO"}),"上调用",e.jsx("code",{children:"read"}),"，渲染窗口只需要把程序输出",e.jsx("code",{children:"printf"}),"到标准输出（之后iTerm程序，会把标准输出的字符绘制到窗口中）。screen可以根据用户需求（e.g. ",e.jsx("code",{children:"screen -a -t 0"}),"）接入（attach）到一个伪终端，也可以离开（detach）当前伪终端，接入其他伪终端，这样就实现了窗口管理和切换。当然，实现screen要考虑的事情比上面说的其实还要多很多，但这篇博客中的实现会保持尽可能简单。myscreen虽然输出没有screen漂亮，不过管理和切换窗口的思想和screen是一致的。"]})]})}function I(){return e.jsxs("div",{children:[e.jsxs("p",{className:"indent",children:["终于，我们可以看看该怎么写代码了。你可以在",e.jsx("a",{href:"https://github.com/brandb97/myscreen",children:"这里"}),"下载一份myscreen的代码。然后运行",e.jsx("code",{children:"make"}),"得到一份可执行文件",e.jsx("code",{children:"myscreen"}),"。"]}),e.jsx(t,{code:`git clone https://github.com/brandb97/myscreen
cd myscreen
make
